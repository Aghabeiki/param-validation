...
let privateZone = module.exports = {
    ...
    matchRequirement: (target, param) => {
        // load all the path
        let errorStack = [];
        let allPath = privateZone.getAllPath(target);
    ...
    matchFieldsDataType: (target, param) => {
        ...
        allParamPath.filter(param => {
            ...
            "use strict";
            let requireParamName = v.config.compareWithFiled.map(elem => {
    ...
    getAllPath(target, basePath) {
        ...
        parentsKey.forEach(parentKey => {
            ...
            let tmp = privateZone.getAllPath(target[parentKey], (basePath === '' ? parentKey : basePath + '.' + parentKey));
    ...
    paramConfigChecker(paramConfig, paramValue) {
        ...
        if (paramConfig.type.toLowerCase() === 'array') {
            ...
            let fn = DataTypeValidator[paramConfig.type.toLowerCase()]
            if (fn) {
